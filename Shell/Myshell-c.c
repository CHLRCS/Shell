...
void cut_Cmd(){
    ...
    /*Reperage et traitement des espace et \0 separant les arguments*/
        ...
        i=traitement;
        cmd_argv[argc]= malloc(MAX_LENGHT_ARGS * sizeof(char));/*Allocation memoire de l'espace necessaire au stockage des arguments coupŽs en mŽmoire*/
        while((user_entry[i] != ' ') && (user_entry[i] != '\0')){
 ...
 * EntrŽe : Chemin externe
 * Sortie : Numero du sous chemin stockŽ dans TryPath
 */
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
int cut_Path(const char* pathEnv)
{
    ...
    while(i<strlen(pathEnv)){
        while(pathEnv[i] == ':')/*Traitement du chemin externe recu , suppression des caracteres non souhaitŽs*/
        {
            i++;
            traitement++;
        }
...
void  exec_Cmd(){
    ...
    /*Analyse du premier argument et execution de la commande souhaitŽe*/
    ...
    if(!strcmp(cmd_argv[0],"cd")){/*Gestion de l'execution de la commande inter "cd"*/
        ...
        goto end;
            ...
            for (i = 1; i < 512; i++) {
                cat_Cmd(cmd_argv[i]);/*Tranmission des arguments en ca de concatŽnation*/
            }
        verified=1;
        goto end;
            ...
            if(!access(fragPath,F_OK))
            {
                if(!access(fragPath,X_OK))
                {
                    ...
                    if(pid == 0){
                        ...
                        /*Gestion des pipes - ExpŽrimentation non aboutie*/
                        if(pipe_Out){
                            dup2(pipe_1[1],STDOUT_FILENO);
                            close(pipe_1[1]);
                        }else if(pipe_In){
...
void change_Directory(char** cmd_argv){
  ...
  if(strcmp(cmd_argv[1],"~") == 0){/*Saisie du caractere ~ pour un retour a la racine*/
    chdir(homedir); /*Mise a jour du répertoire courant vers la racine*/
    } else {
    chdir(cmd_argv[1]); /*Mise a jour du repertoire courant vers répertoire desire*/
    }
...
/* meme procŽdure pour cp_Directory et cp_File */
...
int cp_File(struct stat* source, char* originalPath, char* atPath){
    ...
    int lenght=0;
    char seizure[512];
    if(at ==-1){ /*Erreur lors de la crŽation du fichier copiŽ*/
...
void cp_Directory(struct stat* source, char* originalPath, char* atPath){
    ...
    if(originDir == NULL){/*Si pas de rŽpertoire d'origine*/
